@if($phase === 'closed-folder')
    ...
    <div class="closed-folder-screen">
        <section class="closed-folder-stage">
            <article class="closed-dossier" role="button" tabindex="0"
                ...
                <div class="closed-dossier__ornament">
                    ...
                    <dl class="closed-dossier__identity">
                        ...
                        <div>
                            <dt>Сценарий</dt>
                            <dd>{{ $scenarioInfo['name'] ?? '—' }}</dd>
                        </div>
                        <div>
                            <dt>Сложность</dt>
                            <dd>{{ $this->getDifficultyLabel($game->difficulty) }}</dd>
                        </div>
                    </dl>
            ...
            <button type="button" class="document-action document-action--secondary closed-folder-back"
                    wire:click="backToScenarios">← Назад к сценариям
            </button>
        </section>

        <aside class="closed-folder-sidebar" aria-label="Информация о выбранном сценарии">
            <section class="closed-folder-info">
                <span class="closed-folder-info__icon"><x-game.icon name="scenario"
                                                                    class="closed-folder-info__svg"/></span>
            ...
            <section class="closed-folder-info">
                <span class="closed-folder-info__icon"><x-game.icon name="chart"
                                                                    class="closed-folder-info__svg"/></span>
                <div class="closed-folder-info__copy">
                    <h2>Сложность</h2>
            ...
            <button type="button" class="document-action document-action--primary closed-folder-open"
                    wire:click="openFolder">Открыть дело →
            </button>
        </aside>
    </div>
@elseif($phase === 'introduction')
    ...
    <div class="introduction-layout">
                ...
                <section class="dossier__document-area" aria-label="Область документа">
                    <article class="document-sheet document-sheet--compact document-sheet--introduction">
                        <header class="document-sheet__header">
                            ...
                            <p class="document-sheet__eyebrow">СТРАТЕГИЧЕСКИЕ ШАХМАТЫ: ГОСУПРАВЛЕНИЕ · СЦЕНАРИЙ
                                «{{ $scenarioInfo['name'] ?? '—' }}»</p>
                        ...
                        <div class="document-sheet__body">
                            <p class="introduction-document__lead">Вы только вступили в должность главы округа. На
                                первом совещании команда, оставшаяся от предыдущего руководителя, вводит вас в курс
                                текущего положения дел.</p>
                            ...
                            <section class="document-section">
                                ...
                                <p>В целом округ находится в рабочем состоянии. Основные процессы идут, критических
                                    сбоев нет. Однако многие вопросы приходится вручную доводить до результата, особенно
                                    там, где требуется взаимодействие нескольких подразделений.</p>
                            </section>
                            ...
                            <section class="document-section">
                                ...
                                <p>Жалобы поступают регулярно, но пока без резких всплесков. Основное недовольство
                                    связано не только с качеством услуг, но и с ощущением формальности, непрозрачности и
                                    недостатка обратной связи при принятии решений.</p>
                            </section>
                            ...
                            <section class="document-section">
                                ...
                                <p>Аппарат сохраняет управляемость, но работает с высокой нагрузкой. При росте давления
                                    или возникновении нестандартных ситуаций возможны задержки, несогласованность
                                    действий и управленческие сбои.</p>
                            </section>
                            ...
                            <section class="document-section document-section--conclusion">
                                ...
                                <p>Пока ситуация остаётся стабильной, необходимо выстроить процессы, укрепить
                                    взаимодействие подразделений и восстановить доверие к процедурам.</p>
                            </section>
                        </div>
                    </article>
                </section>
        ...
        <aside class="introduction-sidebar" aria-label="Сведения о вступлении в должность">
            ...
            <section class="closed-folder-info closed-folder-info--ready">
                <span class="closed-folder-info__icon"><x-game.icon name="clock"
                                                                    class="closed-folder-info__svg"/></span>
                <div class="closed-folder-info__copy">
                    <h2>Стадия</h2>
            ...
            <button type="button" class="document-action document-action--primary introduction-accept"
                    wire:click="acceptDuties">Принять дела →
            </button>
        </aside>
    </div>
@else
                    ...
                    <section class="dossier__document-area" aria-label="Область документа">
                        @if($gameplayTab === 'scenario')
                            <article class="document-sheet document-sheet--compact document-sheet--scene">
                                <header class="document-sheet__header">
                                    ...
                                    <p class="document-sheet__subtitle">Сцена {{ $scene->order ?? '' }}
                                        . {{ $monthNames[$scene->order] ?? '' }}. Управленческая ситуация</p>
                                    <div class="document-sheet__divider" aria-hidden="true"><span></span></div>
                                </header>
                                <div class="document-sheet__body">
                                    @if(session()->has('message'))
                                        <div class="document-section__callout"
                                             role="status">{{ session('message') }}</div>
                                    @endif
                                    ...
                                    @if($hasDelayedMessages && !$showDelayedModal)
                                        <aside class="delayed-trigger">
                                            ...
                                            <button type="button" class="document-action document-action--secondary"
                                                    wire:click="openDelayedModal">Открыть
                                            </button>
                                        </aside>
                                    @endif
                                    <section class="document-section">
                                ...
                                <footer class="document-sheet__actions">
                                    <button type="button" class="document-action document-action--text"
                                            wire:click="forceNewGame">Начать заново
                                    </button>
                                    <button type="button" class="document-action document-action--primary"
                                            wire:click="setGameplayTab('decisions')">Перейти к решениям ›
                                    </button>
                                </footer>
                            </article>
                        @elseif($gameplayTab === 'decisions')
                            @if($decisionsDoc === 'result')
                                ...
                                <article
                                    class="document-sheet document-sheet--compact document-sheet--immediate-result">
                                    <header class="document-sheet__header">
                                        <x-game.icon name="emblem" class="document-sheet__emblem"/>
                                        <p class="result-document__context">{{ $resultSnapshot['scene'] ?? '' }}</p>
                                        <h1 class="document-sheet__title">Решение принято</h1>
                                        <p class="document-sheet__subtitle">Непосредственный результат управленческого
                                            действия</p>
                                        <div class="document-sheet__divider" aria-hidden="true"><span></span></div>
                                    </header>
                                    <div class="document-sheet__body result-document__body">
                                        ...
                                        <section class="result-section result-section--positive">
                                            ...
                                            <div class="result-section__body">
                                                <p>Решение применено. Управление переведено на сцену
                                                    «{{ $resultSnapshot['scene'] ?? '' }}».</p>
                                            </div>
                                        </section>
                                                ...
                                                <ul>
                                                    @foreach($inboxMessages as $message)
                                                        ...
                                                        <li role="button"
                                                            ...
                                                            aria-label="Открыть сообщение: {{ $message['actor'] }}. {{ $message['subject'] }}. Статус: {{ $read ? 'Прочитано' : 'Новое' }}">
                                                            <x-game.icon name="mail" class="result-message__icon"/>
                                                            <span>
                                                         <strong>{{ $message['actor'] }}</strong>                                                         <span>{{ $message['subject'] }}</span>
                                                     </span>
                                                            <span
                                                                class="result-message__status @if($read) result-message__status--read @endif">{{ $read ? 'Прочитано' : 'Новое' }}</span>
                                                        </li>
                                                    @endforeach
                                                </ul>
                                            </section>
                                    ...
                                    <footer class="document-sheet__actions result-actions">
                                        <button type="button" class="document-action document-action--secondary"
                                                wire:click="openInbox">Открыть входящие
                                        </button>
                                        <button type="button" class="document-action document-action--text"
                                                wire:click="continueFromResult">Вернуться к сцене
                                        </button>
                                        <button type="button" class="document-action document-action--primary"
                                                wire:click="continueFromResult">Продолжить ›
                                        </button>
                                    </footer>
                                </article>
                            @elseif($decisionsDoc === 'inbox')
                                ...
                                <article class="document-sheet document-sheet--compact document-sheet--inbox">
                                    <header class="document-sheet__header inbox-header">
                                        ...
                                        <p class="inbox-header__supporting">Изучите поступившие сообщения, чтобы понять
                                            позиции участников, внутренние риски и изменение общественной реакции.</p>
                                    ...
                                    <div class="document-sheet__body inbox-document__body">
                                        <section class="inbox-summary" aria-label="Сводка входящих сообщений">
                                            <div>
                                                <strong>{{ $inboxSummary['total'] }}</strong><span>Всего сообщений</span>
                                            </div>
                                            <div><strong>{{ $inboxSummary['unread'] }}</strong><span>Непрочитано</span>
                                            </div>
                                            <div><strong>{{ $inboxSummary['urgent'] }}</strong><span>Срочные</span>
                                            </div>
                                            <div><strong>{{ $inboxSummary['actors'] }}</strong><span>Акторов с сообщениями</span>
                                            </div>
                                        </section>
                                            ...
                                            @forelse($visibleInbox as $message)
                                                ...
                                                <li class="inbox-message">
                                                    <button type="button"
                                                     ...
                                                     <span class="inbox-message__actor">{{ $message['actor'] }}</span>                                                        <strong
                                                                class="inbox-message__subject">{{ $message['subject'] }}</strong>
                                                        <span
                                                            class="inbox-message__preview">{{ $message['preview'] }}</span>
                                                    </span>
                                                        <span class="inbox-message__metadata">
                                                        <span class="inbox-message__time">{{ $message['time'] }}</span>
                                                        <span
                                                            class="inbox-message__status @if($read) inbox-message__status--read @endif">{{ $read ? 'Прочитано' : ($message['urgent'] ? 'Срочно' : 'Новое') }}</span>
                                            ...
                                            @empty
                                                <li class="inbox-empty">
                                                    ...
                                                    <button type="button"
                                                            ...
                                                            wire:click="resetInboxFilters">Сбросить фильтры
                                                    </button>
                                                </li>
                                            @endforelse
                                        </ul>
                                    ...
                                    <footer class="document-sheet__actions inbox-actions">
                                        <button type="button" class="document-action document-action--secondary"
                                                wire:click="backToResult">‹ Назад к результату
                                        </button>
                                        <button type="button" class="document-action document-action--text"
                                                wire:click="markAllInboxRead" @disabled($inboxSummary['unread'] === 0)>
                                    ...
                                    <div class="document-sheet__body decisions-document__body">
                                        <section class="decisions-introduction">
                                            <div>
                                                ...
                                                <p class="decisions-introduction__lead">Выберите один вариант
                                                    управленческого решения. Подтверждение запускает последствия
                                                    хода.</p>
                                            </div>
                                            <div
                                                class="decision-timer @if($decisionExpired) decision-timer--expired @endif"
                                                x-data="{
...
">
                                                ...
                                                <span class="visually-hidden" aria-live="polite"
                                                      x-text="announce"></span>
                                            </div>
                                        </section>

                                        <fieldset class="decision-options">
                                            <legend class="visually-hidden">Выберите один вариант управленческого
                                                решения
                                            </legend>
                                            @foreach ($this->getVisibleChoices() as $option)
                                                ...
                                                <strong class="decision-option__title">{{ $option['title'] }}</strong>
                                                    ...
                                                    <span class="decision-option__marker" aria-hidden="true">@if($selectedChoiceId === $option['id'])
                                                            ✓
                                                        @endif</span>
                                                </button>
                                            @endforeach
                                    ...
                                    <footer class="document-sheet__actions decisions-actions">
                                        <button type="button" class="document-action document-action--secondary"
                                                wire:click="setGameplayTab('scenario')">‹ Назад к сценарию
                                        </button>
                                        <button type="button"
                                                class="document-action document-action--primary decisions-actions__confirm"
                                                wire:click="confirmDecision"
                                            @disabled(!$selectedChoiceId || $decisionExpired)>Подтвердить решение
                                        </button>
                                        <button type="button" class="document-action document-action--text"
                                                wire:click="setGameplayTab('materials')">Открыть материалы ↗
                                        </button>
                                    </footer>
                                </article>
                            @endif
                                ...
                                <header class="materials-header">
                                    <div class="materials-header__heading">
                                        ...
                                        <div>
                                            ...
                                            <p class="materials-header__subtitle">Документы и исходные материалы по
                                                ситуации с уборкой снега</p>
                                        </div>
                                    </div>
                                    <p class="materials-header__support">Изучите документы, чтобы уточнить условия
                                        контракта, распределение ответственности и фактическое исполнение работ.</p>
                                </header>
                                    ...
                                    <div class="materials-toolbar">
                                        <div class="materials-toolbar__controls">
                                            <label class="materials-control">
                                                <span>Категория</span>
                                                <select class="materials-control__select"
                                                        wire:model.live="materialCategory">
                                        ...
                                        <div class="materials-view" role="group" aria-label="Вид каталога">
                                            ...
                                            <button type="button"
                                                    ...
                                                    aria-pressed="{{ $materialView === 'cards' ? 'true' : 'false' }}">
                                                Карточки
                                            </button>
                                            <button type="button"
                                                    class="materials-view__button @if($materialView === 'list') materials-view__button--active @endif"
                                                    wire:click="setMaterialView('list')"
                                                    aria-pressed="{{ $materialView === 'list' ? 'true' : 'false' }}">
                                                Список
                                            </button>
                                        </div>
                                        <p class="materials-toolbar__count" aria-live="polite">
                                            Найдено: {{ count($visibleMaterials) }}</p>
                                    </div>

                                    <section class="materials-catalogue materials-catalogue--{{ $materialView }}"
                                             aria-label="Материалы дела">
                                        @forelse($visibleMaterials as $material)
                                            ...
                                            <article class="material-item @if($studied) material-item--studied @endif">
                                                ...
                                                <span
                                                    class="material-item__importance @if($material['priority'] >= 5) material-item__importance--high @endif">
                                        ...
                                        @empty
                                            <p class="materials-catalogue__empty">По выбранным условиям материалы не
                                                найдены.</p>
                                        @endforelse
                                    </section>
                                </div>
                        ...
                        @elseif($gameplayTab === 'references')
                            ...
                            @endphp
                            <article
                                class="document-sheet document-sheet--compact document-sheet--materials document-sheet--references">
                                <header class="materials-header references-header">
                                    <div class="materials-header__heading">
                                        <x-game.icon name="briefcase"
                                                     class="materials-header__icon references-header__icon"/>
                                        <div>
                                            <h1 class="materials-header__title">Справки по делу</h1>
                                            <p class="materials-header__subtitle">Служебная и аналитическая информация
                                                для принятия решения</p>
                                        </div>
                                    </div>
                                    <p class="materials-header__support">Изучите подготовленные справки, чтобы оценить
                                        ресурсы, ограничения, риски и позиции участников ситуации.</p>
                                </header>
                                    ...
                                    <div class="materials-toolbar references-toolbar">
                                        <div class="materials-toolbar__controls">
                                            <label class="materials-control">
                                                <span>Категория</span>
                                                <select class="materials-control__select"
                                                        wire:model.live="referenceCategory">
                                        ...
                                        <div class="materials-view" role="group" aria-label="Вид каталога справок">
                                            ...
                                            <button type="button"
                                                    ...
                                                    aria-pressed="{{ $referenceView === 'cards' ? 'true' : 'false' }}">
                                                Карточки
                                            </button>
                                            <button type="button"
                                                    class="materials-view__button @if($referenceView === 'list') materials-view__button--active @endif"
                                                    wire:click="setReferenceView('list')"
                                                    aria-pressed="{{ $referenceView === 'list' ? 'true' : 'false' }}">
                                                Список
                                            </button>
                                        </div>
                                        <p class="materials-toolbar__count" aria-live="polite">
                                            Найдено: {{ count($visibleReferences) }}</p>
                                    </div>

                                    <section
                                        class="materials-catalogue materials-catalogue--{{ $referenceView }} references-catalogue references-catalogue--{{ $referenceView }}"
                                        aria-label="Справки по делу">
                                        @forelse($visibleReferences as $reference)
                                            @php $studied = in_array($reference['id'], $studiedReferenceIds, true); @endphp
                                            <article
                                                class="material-item reference-card reference-card--{{ $reference['category'] }} @if($studied) material-item--studied reference-card--studied @endif">
                                                <x-game.icon name="{{ $reference['icon'] }}"
                                                             class="material-item__icon reference-card__icon"/>
                                                <div class="material-item__copy">
                                                ...
                                                <footer class="material-item__footer">
                                                <span
                                                    class="material-item__importance @if($reference['importance'] === 'Высокая') material-item__importance--high @endif">
                                        ...
                                        @empty
                                            <p class="materials-catalogue__empty">Справки по выбранному фильтру
                                                отсутствуют.</p>
                                        @endforelse
                                    </section>
                                </div>
                        ...
                        @elseif($gameplayTab === 'media')
                            ...
                            @endphp
                            <article
                                class="document-sheet document-sheet--compact document-sheet--materials document-sheet--media">
                                <header class="materials-header media-header">
                                    <div class="materials-header__heading">
                                        <x-game.icon name="broadcast"
                                                     class="materials-header__icon media-header__icon"/>
                                        <div>
                                            <h1 class="materials-header__title">СМИ и информационное поле</h1>
                                            <p class="materials-header__subtitle">Публикации, запросы и сигналы по
                                                ситуации с уборкой снега</p>
                                        </div>
                                    </div>
                                    <p class="materials-header__support">Изучите информационный фон, чтобы понять
                                        уровень общественного внимания, позицию редакций и динамику публичного
                                        давления.</p>
                                </header>
                                    ...
                                    <div class="materials-toolbar media-toolbar">
                                        ...
                                        <div class="materials-view" role="group" aria-label="Вид медиакаталога">
                                            ...
                                            <button type="button"
                                                    ...
                                                    aria-pressed="{{ $mediaView === 'cards' ? 'true' : 'false' }}">
                                                Карточки
                                            </button>
                                            <button type="button"
                                                    class="materials-view__button @if($mediaView === 'list') materials-view__button--active @endif"
                                                    wire:click="setMediaView('list')"
                                                    aria-pressed="{{ $mediaView === 'list' ? 'true' : 'false' }}">Список
                                            </button>
                                        </div>
                                        <p class="materials-toolbar__count" aria-live="polite">
                                            Найдено: {{ count($visibleMedia) }}</p>
                                    </div>

                                    <section
                                        class="materials-catalogue materials-catalogue--{{ $mediaView }} media-catalogue media-catalogue--{{ $mediaView }}"
                                        aria-label="СМИ и информационное поле">
                                        @forelse($visibleMedia as $item)
                                            @php $reviewed = in_array($item['id'], $reviewedMediaIds, true); @endphp
                                            <article
                                                class="material-item media-card media-card--{{ $item['type'] }} @if($reviewed) material-item--studied media-card--reviewed @endif">
                                                <x-game.icon name="{{ $item['icon'] }}"
                                                             class="material-item__icon media-card__icon"/>
                                                <div class="material-item__copy">
                                                    ...
                                                    <div class="media-card__badges">
                                                        <span
                                                            class="media-card__badge media-card__badge--tone-{{ mb_strtolower($item['tone']) }}">Тон: {{ $item['tone'] }}</span>
                                                        <span class="media-card__badge media-card__badge--resonance">Резонанс: {{ $item['resonance'] }}</span>
                                                        <span
                                                            class="media-card__badge media-card__badge--status">{{ $item['status'] }}</span>
                                                    </div>
                                                </div>
                                                <footer class="material-item__footer">
                                                <span
                                                    class="material-item__importance @if($item['priority'] === 'Высокая') material-item__importance--high @endif">
                                        ...
                                        @empty
                                            <p class="materials-catalogue__empty">Материалы по выбранному фильтру
                                                отсутствуют.</p>
                                        @endforelse
                                    </section>
                                </div>
                                <footer class="document-sheet__actions materials-actions media-actions">
                                    ...
                                    <button type="button" class="document-action document-action--text"
                                            wire:click="setGameplayTab('references')">Открыть справки
                                    </button>
                                    <button type="button" class="document-action document-action--primary"
                                            wire:click="setGameplayTab('decisions')">Перейти к решениям ›
                                    </button>
                        ...
                        @elseif($gameplayTab === 'appeals')
                            ...
                            @endphp
                            <article
                                class="document-sheet document-sheet--compact document-sheet--materials document-sheet--appeals">
                                <header class="materials-header appeals-header">
                                    <div class="materials-header__heading">
                                        ...
                                        <div>
                                            ...
                                            <p class="materials-header__subtitle">Жалобы, повторные обращения и сигналы
                                                по ситуации с уборкой снега</p>
                                        </div>
                                    </div>
                                    <p class="materials-header__support">Изучите обращения, чтобы определить основные
                                        темы недовольства, проблемные территории и динамику повторных жалоб.</p>
                                </header>
                                    ...
                                    <div class="materials-toolbar appeals-toolbar">
                                        <div class="materials-toolbar__controls">
                                            <label class="materials-control">
                                                <span>Категория</span>
                                                <select class="materials-control__select"
                                                        wire:model.live="appealCategory">
                                        ...
                                        <div class="materials-view" role="group" aria-label="Вид каталога обращений">
                                            ...
                                            <button type="button"
                                                    ...
                                                    aria-pressed="{{ $appealView === 'cards' ? 'true' : 'false' }}">
                                                Карточки
                                            </button>
                                            <button type="button"
                                                    class="materials-view__button @if($appealView === 'list') materials-view__button--active @endif"
                                                    wire:click="setAppealView('list')"
                                                    aria-pressed="{{ $appealView === 'list' ? 'true' : 'false' }}">
                                                Список
                                            </button>
                                        </div>
                                        <p class="materials-toolbar__count" aria-live="polite">
                                            Найдено: {{ count($visibleAppeals) }}</p>
                                    </div>

                                    <section
                                        class="materials-catalogue materials-catalogue--{{ $appealView }} appeals-catalogue appeals-catalogue--{{ $appealView }}"
                                        aria-label="Обращения жителей">
                                        @forelse($visibleAppeals as $appeal)
                                            @php $reviewed = in_array($appeal['id'], $reviewedAppealIds, true); @endphp
                                            <article
                                                class="material-item appeal-card appeal-card--{{ $appeal['type'] }} @if($reviewed) material-item--studied appeal-card--reviewed @endif">
                                                <x-game.icon name="{{ $appeal['icon'] }}"
                                                             class="material-item__icon appeal-card__icon"/>
                                                <div class="material-item__copy">
                                                    ...
                                                    <div class="appeal-card__badges">
                                                        ...
                                                        <span
                                                            class="appeal-card__badge appeal-card__badge--status">{{ $appeal['status'] }}</span>
                                                    </div>
                                                </div>
                                                <footer class="material-item__footer">
                                                <span
                                                    class="material-item__importance @if($appeal['priorityValue'] >= 3) material-item__importance--high @endif">
                                        ...
                                        @empty
                                            <p class="materials-catalogue__empty">Обращения по выбранному фильтру
                                                отсутствуют.</p>
                                        @endforelse
                                    </section>
                                </div>
                                <footer class="document-sheet__actions materials-actions appeals-actions">
                                    ...
                                    <button type="button" class="document-action document-action--text"
                                            wire:click="setGameplayTab('media')">Открыть СМИ
                                    </button>
                                    <button type="button" class="document-action document-action--primary"
                                            wire:click="setGameplayTab('decisions')">Перейти к решениям ›
                                    </button>
                            ...
                            <article class="document-sheet document-sheet--compact document-sheet--annual-placeholder">
                                ...
                                <div class="document-sheet__body annual-placeholder__body">
                                    <section class="annual-placeholder__note">
                                        ...
                                        <p>Раздел «{{ $tabTitles[$gameplayTab] ?? '' }}» будет реализован на следующем
                                            шаге.</p>
                                    </section>
                                </div>
                                <footer class="document-sheet__actions">
                                    <button type="button" class="document-action document-action--secondary"
                                            wire:click="setGameplayTab('scenario')">‹ Вернуться к сценарию
                                    </button>
                                </footer>
                            </article>
                        @endif
                                    ...
                                    @foreach($currentState as $key => $value)
                                        <div class="metric-row metric-row--{{ $this->getMetricTone($value) }}">
                                            <div class="metric-row__heading">
                                                <span
                                                    class="metric-row__label">{{ $this->getParameterLabel($key) }}</span>
                                            ...
                                            <div class="metric-row__track" role="progressbar"
                                                ...
                                                <span class="metric-row__bar"
                                                      style="--metric-value: {{ $value }}%"></span>
                                            </div>
                                        </div>
                                    @endforeach
                                </div>
                                            ...
                                            <div>
                                                ...
                                                <div class="event-history__label">
                                                    @if($event['is_actor_event'])
                                                        ↳ {{ $event['actor_name'] ?? 'Актор' }}
                                                        — {{ $event['actor_type'] ?? $event['event_name'] }}
                                                    @else
                                                        {{ $event['event_name'] }}
                                                    @endif
                                                </div>
                            ...
                            <ul class="actor-message-list">
                                @if($hasDelayedMessages)
                                    <li class="actor-message-row actor-message-row--unread">
                                        <button type="button" class="actor-message-row__button"
                                                wire:click="openDelayedModal" aria-label="Открыть новые сообщения">
                                ...
                                @foreach($sceneActors as $actor)
                                    <li class="actor-message-row">
                                        <x-game.icon name="{{ $this->getActorIcon($actor['name']) }}"
                                                     class="actor-message-row__icon"/>
        ...
        @if($showMessageModal && !empty($currentModalMessage))
         ...
         $msgSender   = $currentModalMessage['actor'] ?? $currentModalMessage['sender'] ?? 'Актор';
         $msgSubject  = $currentModalMessage['subject'] ?? (mb_strlen($msgText) > 80 ? mb_substr($msgText, 0, 80) . '…' : $msgText);
         $isPlayerOrigin = str_contains($msgSender, 'действий игрока');
                ...
                <div class="message-toast" role="status" x-data="{ visible: true }" x-show="visible">
                    <div class="message-toast__open">
                     ...
                     <strong>Новое письмо</strong>
                                          <span>{{ $isPlayerOrigin ? $msgSender : $msgSender . ' направил сообщение' }}</span>
                 </span>
                    </div>
                    <button type="button" class="message-toast__dismiss" x-on:click="visible = false"
                            aria-label="Скрыть уведомление о новом письме">×
                    </button>
                </div>
            @endif
            ...
            <div class="incoming-message-backdrop"
              ...
              destroy() { document.body.classList.remove('message-modal-open'); }
                ...
                <article class="incoming-message" role="dialog" aria-modal="true"
                         aria-labelledby="incoming-message-title">
                    <header class="incoming-message__header">
                        <span aria-hidden="true"></span>
                        <h2 id="incoming-message-title">Входящее сообщение</h2>
                        <button type="button" class="incoming-message__close" wire:click="closeMessageModal"
                                aria-label="Закрыть входящее сообщение">×
                        </button>
                    </header>
                    ...
                    <div class="incoming-message__content">
                        <dl class="incoming-message__metadata">
                            ...
                            <dd>{{ $msgSubject }}</dd>
                            <dt>Приоритет:</dt>
                            <dd><span class="incoming-message__priority-dot {{ $priorityDot }}"
                                      aria-hidden="true"></span>{{ $msgPriority }}</dd>
                            <dt>Время:</dt>
                            <dd>{{ $msgTime }}</dd>
                        </dl>
                    ...
                    <footer class="incoming-message__footer">
                        @if($messageModalFromInbox)
                            <button type="button" class="document-action document-action--secondary"
                                    wire:click="closeMessageModal">Закрыть
                            </button>
                            <button type="button" class="document-action document-action--primary"
                                    wire:click="markCurrentMessageRead">Отметить как прочитанное
                            </button>
                        @else
                            <button type="button" class="document-action document-action--secondary"
                                    wire:click="closeMessageModal">Закрыть
                            ...
                            <button type="button" class="document-action document-action--primary"
                                    wire:click="closeMessageModalToScene">Перейти к сцене
                            </button>
                        @endif
                    </footer>
                </article>
        ...
        @if($showDelayedModal && !empty($delayedMessages))
            ...
            <div class="delayed-effect-backdrop" wire:click.self="closeDelayedModal"
                 x-on:keydown.escape.window="$wire.closeDelayedModal()">
                <article class="delayed-effect" role="dialog" aria-modal="true" aria-labelledby="delayed-effect-title"
                         wire:click.stop>
                    <header class="delayed-effect__header">
                        ...
                        <button type="button" class="delayed-effect__close" wire:click="closeDelayedModal"
                                aria-label="Закрыть отложенное последствие">×
                        </button>
                    </header>
                    ...
                    <div class="delayed-effect__content">
                        <dl class="delayed-effect__metadata">
                            <div>
                                <dt>Источник</dt>
                                <dd>{{ ($effect['source_month'] ?? null) ? $effect['source_month'] . '. ' . ($effect['source_scene'] ?? '') : 'Ранее принятое решение' }}</dd>
                            </div>
                            <div>
                                <dt>Решение</dt>
                                <dd>{{ $effect['source_decision'] ?? 'Выбор, сделанный ранее' }}</dd>
                            </div>
                            <div>
                                <dt>Прошло времени</dt>
                                <dd>{{ $effect['delay_label'] ?? 'Несколько недель' }}</dd>
                            </div>
                            <div>
                                <dt>Статус</dt>
                                <dd>{{ $effect['status'] ?? 'Проявилось' }}</dd>
                            </div>
                        </dl>
                        ...
                        <section class="delayed-effect__impact">
                            ...
                            <p>Текущая сцена учитывает проявившиеся последствия. Ознакомьтесь с обновлениями и
                                продолжите управление.</p>
                        </section>
                    </div>
                    <footer class="delayed-effect__footer">
                        <button type="button" class="document-action document-action--secondary"
                                wire:click="closeDelayedModal">Закрыть
                        </button>
                        <button type="button" class="document-action document-action--primary"
                                wire:click="closeDelayedModal">Продолжить
                        </button>
                    </footer>
                </article>
            </div>
        ...
        @if($activeMaterial = $this->getActiveMaterial())
            <div class="material-modal-backdrop" wire:click="closeMaterialModal"
                 x-on:keydown.escape.window="$wire.closeMaterialModal()">
                <section class="material-modal" role="dialog" aria-modal="true" aria-labelledby="material-modal-title"
                    ...
                    <p class="material-modal__metadata">{{ $activeMaterial['fileType'] }}
                        · {{ $activeMaterial['categoryLabel'] }} · {{ implode(' · ', $activeMaterial['metadata']) }}</p>
                    <div class="material-modal__actions">
                        <button type="button" class="document-action document-action--secondary"
                                wire:click="closeMaterialModal">Закрыть
                        </button>
                        @php $studiedActive = in_array($activeMaterial['id'], $studiedMaterialIds, true); @endphp
                        <button type="button" class="document-action document-action--primary"
                                wire:click="markMaterialStudied" @disabled($studiedActive)>
        ...
        @if($activeReference = $this->getActiveReference())
            <div class="material-modal-backdrop reference-modal-backdrop" wire:click="closeReferenceModal"
                 x-on:keydown.escape.window="$wire.closeReferenceModal()">
                <section class="material-modal reference-modal" role="dialog" aria-modal="true"
                         aria-labelledby="reference-modal-title" wire:click.stop>
                    ...
                    <p class="material-modal__metadata">{{ $activeReference['categoryLabel'] }}
                        · {{ $activeReference['source'] }} · {{ implode(' · ', $activeReference['metadata']) }}</p>
                    <div class="material-modal__actions">
                        <button type="button" class="document-action document-action--secondary"
                                wire:click="closeReferenceModal">Закрыть
                        </button>
                        @php $studiedRef = in_array($activeReference['id'], $studiedReferenceIds, true); @endphp
                        <button type="button" class="document-action document-action--primary"
                                wire:click="markReferenceStudied" @disabled($studiedRef)>
        ...
        @if($activeMedia = $this->getActiveMedia())
            <div class="material-modal-backdrop media-modal-backdrop" wire:click="closeMediaModal"
                 x-on:keydown.escape.window="$wire.closeMediaModal()">
                <section class="material-modal media-modal" role="dialog" aria-modal="true"
                         aria-labelledby="media-modal-title" wire:click.stop>
                    <h2 id="media-modal-title">{{ $activeMedia['type'] === 'request' ? 'Запрос редакции' : 'Просмотр публикации' }}</h2>
                    <h3>{{ $activeMedia['title'] }}</h3>
                    <p class="media-modal__source">{{ $activeMedia['categoryLabel'] }}
                        · {{ $activeMedia['source'] }}</p>
                    <p>{{ $activeMedia['description'] }}</p>
                    <p class="material-modal__metadata">{{ implode(' · ', $activeMedia['metadata']) }} ·
                        Тон: {{ $activeMedia['tone'] }} · Резонанс: {{ $activeMedia['resonance'] }}
                        · {{ $activeMedia['status'] }}</p>
                    <div class="material-modal__actions">
                        <button type="button" class="document-action document-action--secondary"
                                wire:click="closeMediaModal">Закрыть
                        </button>
                        @php $reviewedActive = in_array($activeMedia['id'], $reviewedMediaIds, true); @endphp
                        <button type="button" class="document-action document-action--primary"
                                wire:click="markMediaReviewed" @disabled($reviewedActive)>
        ...
        @if($activeAppeal = $this->getActiveAppeal())
            <div class="material-modal-backdrop appeal-modal-backdrop" wire:click="closeAppealModal"
                 x-on:keydown.escape.window="$wire.closeAppealModal()">
                <section class="material-modal appeal-modal" role="dialog" aria-modal="true"
                         aria-labelledby="appeal-modal-title" wire:click.stop>
                    <h2 id="appeal-modal-title">Карточка обращения</h2>
                    <h3>{{ $activeAppeal['title'] }}</h3>
                    <p class="appeal-modal__source">{{ $activeAppeal['categoryLabel'] }}
                        · {{ $activeAppeal['source'] }}</p>
                    <p>{{ $activeAppeal['description'] }}</p>
                    <p class="appeal-modal__address">Адрес: {{ $activeAppeal['address'] }}</p>
                    <p class="material-modal__metadata">{{ implode(' · ', $activeAppeal['metadata']) }} ·
                        Приоритет: {{ $activeAppeal['priority'] }} · Статус: {{ $activeAppeal['status'] }}</p>
                    <div class="material-modal__actions">
                        <button type="button" class="document-action document-action--secondary"
                                wire:click="closeAppealModal">Закрыть
                        </button>
                        @php $reviewedActive = in_array($activeAppeal['id'], $reviewedAppealIds, true); @endphp
                        <button type="button" class="document-action document-action--primary"
                                wire:click="markAppealReviewed" @disabled($reviewedActive)>
                            {{ $reviewedActive ? 'Изучено' : 'Отметить как изученное' }}
                        </button>
                    </div>
                </section>
