<div>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            Управление связями компаний и сценариев
        </h2>
    </x-slot>
                ...
                @if($showForm)
                    <div class="bg-gray-50 shadow-md rounded px-8 pt-6 pb-8 mb-4 border">
                        <h3 class="text-lg font-semibold mb-4">
                            {{ $editingId ? 'Редактирование связи' : 'Новая связь' }}
                        </h3>
                        ...
                        <form wire:submit="save">
                            <div class="mb-4">
                                ...
                                <select wire:model="company_id" id="company_id"
                                    ...
                                    @foreach($companies as $company)
                                        <option value="{{ $company->id }}">{{ $company->name }} ({{ $company->difficulty }})</option>
                                    @endforeach
                                </select>
                            ...
                            <div class="mb-4">
                                ...
                                <select wire:model="scenario_id" id="scenario_id"
                                    ...
                                    @foreach($scenarios as $scenario)
                                        <option value="{{ $scenario->id }}">{{ $scenario->name }} ({{ $scenario->difficulty }})</option>
                                    @endforeach
                                </select>
                            ...
                            <div class="mb-4">
                                <label class="block text-gray-700 text-sm font-bold mb-2" for="order">Порядок</label>
                                <input wire:model="order" type="number" id="order" min="0"
                                       class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                                       placeholder="Порядок сценария">
                ...
                <div class="overflow-x-auto">
                    <table class="min-w-full bg-white">
                        ...
                        <tr class="bg-gray-200 text-gray-600 uppercase text-sm leading-normal">
                            ...
                            <th class="py-3 px-6 text-center">Порядок</th>
                            <th class="py-3 px-6 text-center">Действия</th>
                        </tr>
                        </thead>
                        <tbody class="text-gray-600 text-sm">
                        @foreach($companyScenarios as $companyScenario)
                            <tr class="border-b border-gray-200 hover:bg-gray-100">
                                <td class="py-3 px-6">{{ $companyScenario->id }}</td>
                                <td class="py-3 px-6 font-medium">{{ $companyScenario->company->name ?? 'N/A' }}</td>
                                <td class="py-3 px-6">{{ $companyScenario->scenario->name ?? 'N/A' }}</td>
                                <td class="py-3 px-6 text-center">{{ $companyScenario->order }}</td>
                                <td class="py-3 px-6 text-center">
                                    <button wire:click="edit({{ $companyScenario->id }})"
                                            class="bg-yellow-500 hover:bg-yellow-700 text-white font-bold py-1 px-3 rounded text-xs mr-2">
                                        Редактировать
                                    </button>
                                    <button wire:click="delete({{ $companyScenario->id }})"
